...
class UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should not create user with invalid username format" do
    assert_no_difference("User.count") do
      post user_url, params: {
        user: {
          username: "test user!",
  ...
  test "should not create user with reserved username" do
    assert_no_difference("User.count") do
      post user_url, params: {
        user: {
          username: User::USERNAME_BLACKLIST.sample,
          email_address:,
          password:,
          password_confirmation: password
        }
  ...
  test "should not create user with username too short" do
    assert_no_difference("User.count") do
      post user_url, params: {
        user: {
          username: "a",
  ...
  test "should not create user with username too long" do
    assert_no_difference("User.count") do
      post user_url, params: {
        user: {
          username: "a" * (User::USERNAME_MAX_LENGTH + 1),
          email_address:,
          password:,
          password_confirmation: password
        }
  ...
  # Test username and email normalization
  test "should normalize username and email on creation" do
    post user_url, params: {
      user: {
        username: "  TestUser  ",
        email_address: "  Test@EXAMPLE.COM  ",
